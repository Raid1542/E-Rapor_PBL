  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!taRes.ok) {
          const errorText = await taRes.text();
          console.error('Error dari server tahun ajaran:', errorText);
          throw new Error('Gagal ambil tahun ajaran aktif');
        }
        const taData = await taRes.json();
        // Periksa apakah respons berisi data atau langsung objek
