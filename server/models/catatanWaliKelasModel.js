...
const catatanWaliKelasModel = {
    ...
    async getCatatanByKelas(kelasId, tahunAjaranId, semester) {  // ✅ tambahkan parameter semester
        const [rows] = await db.execute(`
            SELECT 
                s.id_siswa AS id,
        ...
        `, [semester, kelasId, tahunAjaranId]);  // urutan sesuai placeholder
        return rows;
    },

    async upsertCatatan(siswaId, kelasId, tahunAjaranId, semester, catatan_pts, catatan_pas, naik_tingkat) {  
        await db.execute(`
            INSERT INTO catatan_wali_kelas 
                (siswa_id, kelas_id, tahun_ajaran_id, semester, catatan_pts, catatan_pas, naik_tingkat)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                ...
                updated_at = NOW()
        `, [siswaId, kelasId, tahunAjaranId, semester, catatan_pts, catatan_pas, naik_tingkat]);
    }
};
